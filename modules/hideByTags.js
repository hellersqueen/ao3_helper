...
;(function(){
  ...
  const { $, $$, onReady, observe, css, on, debounce } = AO3H.util || {};
...
/* AO3 commas handling only where we manage them */
...
.commas   li > a.tag.${NS}-tag-wrap{ font-size:.92em; line-height:1.15; }
ul.commas li::after, ol.commas li::after, .commas li::after{ font-size:.92em; }

/* ===================== MANAGER PANEL / TOAST ===================== */
.${NS}-toast { position:fixed; bottom:10px; left:50%; transform:translateX(-50%);
  background:rgba(0,0,0,.75); color:#fff; padding:6px 10px; border-radius:10px; font-size:11px; z-index:999999;
...
/* (rest of panel styles unchanged from your original) */
  `;
  ...
  function openManager(){
    ...
    async function reload(){
      ...
      for (const [gname, tags] of entries) {
        ...
        // --- Rows (hidden/shown via [aria-expanded] CSS) ---
        ...
        for (const tag of tags) {
          ...
          label.textContent = tag; // read-only

          // Open mini group picker
          ...
          del.addEventListener('click', async () => {
            if (!confirm(`Are you sure you want to delete the tag "${tag}" from your hidden list?`)) {
              return;
            }
            const cur = await getHidden();
