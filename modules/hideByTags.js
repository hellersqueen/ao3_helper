...
(function () {
  ...
  function canonicalFromAnchor(a) {
    try {
      ...
      s = s.replace(/\*a\*/gi, '&').replace(/\*s\*/gi, '/'); // AO3 star-codes
  ...
  async function setHidden(arr) {
    ...
    if (LS_MIRROR) { try { localStorage.setItem(LS_KEY, JSON.stringify(cleaned)); } catch {} }
    return cleaned;
  }
  ...
  async function setGroupsMap(map) {
    ...
    for (const [k, v] of Object.entries(map || {})) {
      ...
      if (allowed.has(key)) cleaned[key] = String(v || '').trim();
    }
    await Storage.set(TM_KEY_GROUPS, cleaned);
    if (LS_MIRROR) { try { localStorage.setItem(LS_KEY_GROUPS, JSON.stringify(cleaned)); } catch {} }
    return cleaned;
  }
  ...
  // --- V1 CSS (exact visuals) ---
  css`
/* ===================== FOLD / CUT ===================== */
.${NS}-fold{position:relative;display:flex!important;align-items:center;gap:.5rem;justify-content:flex-start;width:98%;min-height:14px;padding:.65rem .8rem;border:1px dashed #bdbdbd;border-radius:8px;background:#fee9e9;font-size:.75rem;color:#333;cursor:pointer;user-select:none;touch-action:manipulation;}
...
.${NS}-hide-ico{position:absolute;right:.2em;top:50%;transform:translateY(-50%);width:1em;height:1em;line-height:1em;text-align:center;font-size:.9em;border:1px solid #bbb;border-radius:50%;background:#fff;opacity:0;pointer-events:none;transition:opacity .15s,transform .15s;z-index:2;}
...
ul.commas li> a.tag.${NS}-tag-wrap,ol.commas li> a.tag.${NS}-tag-wrap,.commas li> a.tag.${NS}-tag-wrap{font-size:.92em;line-height:1.15;}
ul.commas li::after,ol.commas li::after,.commas li::after{font-size:.92em;}
a.tag.${NS}-tag-wrap .${NS}-hide-ico{font-size:.9em;}

/* ===================== MANAGER PANEL ===================== */
.${NS}-mgr-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:999998;}
.${NS}-mgr{position:fixed;top:10vh;left:50%;transform:translateX(-50%);background:#fff;color:#000;border:1px solid #e5e7eb;border-radius:12px;padding:10px;z-index:999999;box-shadow:0 16px 40px rgba(2,15,35,.12);font:12px/1.3 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;display:grid;gap:8px;max-height:82vh;overflow:auto;}
...
.${NS}-ul-btn{height:28px;padding:0 10px;border-radius:8px;border:1px solid #cfd6e4;background:#f5f7fb;font-size:12px;cursor:pointer;transition:background .15s,transform .12s,border-color .15s;}
...
/* ===================== TOAST ===================== */
.${NS}-toast{position:fixed;bottom:10px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,.75);color:#fff;padding:6px 10px;border-radius:10px;font-size:11px;z-index:999999;opacity:0;transition:opacity .15s ease;pointer-events:none;}

/* ===================== MINI GROUP PICKER ===================== */
.${NS}-gp-pop{position:absolute;z-index:1000000;min-width:180px;max-width:260px;max-height:50vh;overflow:auto;background:#fff;border:1px solid #bbb;border-radius:8px;box-shadow:0 6px 16px rgba(0,0,0,.2);padding:6px;font:12px/1.3 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;}
...
/* ===================== RESPONSIVE ===================== */
@media (max-width:720px){ .${NS}-ul-head{grid-template-columns:1fr;} .${NS}-ul-actions{justify-content:flex-start;} }
`;
  ...
  function openManager() {
    ...
    async function openGroupPicker(anchorBtn, currentTag, currentGroup, getHidden, getGroupsMap, setGroupsMap, onApplied) {
      ...
      Object.assign(pop.style, { left: `${Math.round(window.scrollX + r.left)}px`, top: `${Math.round(window.scrollY + r.bottom + 6)}px` });
      ...
      input.addEventListener('keydown', (e) => { if (e.key === 'Enter') btnApply.click(); });
      input.focus();
    }
    ...
    let searchTimer = 0;
    const onSearch = (cb) => { clearTimeout(searchTimer); searchTimer = setTimeout(cb, 150); };
    ...
    async function reload() {
      ...
      const filtered = hidden.filter(t => !qn || toNorm(t).includes(qn) || toNorm(groupsMap[t] || '').includes(qn));
      ...
      for (const [gname, tags] of entries) {
        ...
        for (const tag of tags) {
          ...
          gbtn.addEventListener('click', () => {
            openGroupPicker(gbtn, tag, (groupsMap[tag] || '').trim(), getHidden, getGroupsMap, setGroupsMap, reload);
          });
    ...
    box.querySelector('.import').addEventListener('click', async () => {
      ...
      input.addEventListener('change', async () => {
        ...
        } catch (err) { alert('Invalid JSON file for tags.\n' + (err?.message || '')); }
      });
      input.click();
    });
    ...
    box.querySelector('.importg').addEventListener('click', async () => {
      ...
      input.addEventListener('change', async () => {
        ...
        } catch (err) { alert('Invalid JSON file for groups mapping.\n' + (err?.message || '')); }
      });
      input.click();
    });
  ...
  MOD.init = async (flags) => {
    ...
    onReady(() => {
      // Always wire manager opener (core menu dispatches this)
      document.addEventListener(`${NS}:open-hide-manager`, openManager);

      // ⬇️ Tampermonkey popup menu — same as your working version (includes alert)
      if (typeof GM_registerMenuCommand === 'function') {
        GM_registerMenuCommand('AO3 Helper: Manage hidden tags…', openManager);

        // Keep this if you want the alert popup:
        ...
        // Optional extras (export/import) — comment out if you don’t need them here
