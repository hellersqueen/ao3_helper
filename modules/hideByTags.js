...
(function(){
  ...
  // --- storage keys (GM + localStorage mirrors) ---
...
/* AO3 commas control (avoid orphan comma) */
...
@media (max-width: 720px){
  ...
  // --- inline icon + AO3 commas handling ---
  ...
  function openManager(){
    ...
    const backdrop = document.createElement('div'); backdrop.className = `${NS}-mgr-backdrop`;
    const box = document.createElement('div'); box.className = `${NS}-mgr`;
    box.innerHTML = `
    ...
    async function reload(){
      ...
      for (const [gname, tags] of entries) {
        ...
        const chev = document.createElement('span'); chev.className = `${NS}-ul-chevron`; chev.textContent = '';
        const glabel = document.createElement('span'); glabel.className = `${NS}-ul-glabel`; glabel.textContent = `${gname || ''} — ${tags.length}`;

        function toggleGroup(){
          const nowExpanded = block.getAttribute('aria-expanded') !== 'true';
          block.setAttribute('aria-expanded', String(nowExpanded));
          if (nowExpanded) collapsedSet.delete(gname); else collapsedSet.add(gname);
        ...
        for (const tag of tags) {
          const row = document.createElement('div'); row.className = `${NS}-ul-row`;
          const label = document.createElement('span'); label.className = `${NS}-ul-tag`; label.textContent = tag;

          const gbtn = document.createElement('button');
          gbtn.className = `${NS}-ul-gbtn`; gbtn.type = 'button';
          gbtn.title = 'Changer de groupe'; gbtn.setAttribute('aria-label','Changer de groupe'); gbtn.textContent = '📁';
          gbtn.addEventListener('click', () => {
            openGroupPicker(gbtn, tag, (groupsMap[tag] || '').trim(), getHidden, getGroupsMap, setGroupsMap, reload);
          });

          const del = document.createElement('button');
          del.className = `${NS}-ul-btn ${NS}-ul-del`; del.textContent = '🗑️'; del.title = 'Supprimer ce tag caché';
  ...
  MOD.init = async (flags)=>{
    ...
    onReady(() => {
      // Manager opener (core header menu emits this)
      document.addEventListener(`${NS}:open-hide-manager`, openManager);

      // OPTIONAL: Tampermonkey menu items (restores v1 alert behavior)
