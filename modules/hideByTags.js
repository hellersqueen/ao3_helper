/* -------- MODULE 3 : HIDE TAGS (Ultra-Light) -------- */

const HideByTags = (() => {
  let enabled = false;
  let delegatesAttached = false;
  let observerActive = false; // ensures we wire the observer only once
...
a.tag.${NS}-tag-wrap .${NS}-tag-comma {
  ...
  // ✅ Affichage du bandeau (avec raisons) — virgules safe dans <strong>
