;(function(){
  ...
  // ——— Import/Export chooser (Module 5 hooks) ———
  function ensureHiddenWorksChooser(){
    ...
    css`
      ...
      #ao3h-ie-dialog { border:1px solid #bfc7cf; border-radius:10px; padding:16px 16px 14px;
  ...
  function buildSettingsUI(flags){
    css`
      .${NS}-navlink{ color:#fff; text-decoration:none; padding:.5em .8em; display:inline-block; transition:background-color .2s; cursor:default; pointer-events:none; }
      .${NS}-root:hover .${NS}-navlink, .${NS}-root:focus-within .${NS}-navlink, .${NS}-root.${NS}-open .${NS}-navlink{ background-color: rgba(255,255,255,0.15); }
    ...
    // Hidden tags manager (module 3 listens to this event)
    {
      const liM = document.createElement('li');
      const a = document.createElement('a');
      a.href = '#'; a.innerHTML = `<span>Manage hidden tags…</span>`;
      a.addEventListener('click', (e)=>{ e.preventDefault(); document.dispatchEvent(new CustomEvent(`${NS}:open-hide-manager`)); closeMenu(); });
      liM.appendChild(a); menu.appendChild(liM);
    }
    // Import/Export chooser for Module 5
    {
      ensureHiddenWorksChooser();
      const liE = document.createElement('li');
      const a = document.createElement('a');
      a.href = '#'; a.innerHTML = `<span>Hidden works…</span><span class="${NS}-kbd">Import / Export</span>`;
      a.addEventListener('click', (e)=>{ e.preventDefault(); const dlg = document.getElementById('ao3h-ie-dialog'); try { dlg.showModal(); } catch { dlg.setAttribute('open',''); } closeMenu(); });
      liE.appendChild(a); menu.appendChild(liE);
    }
  ...
  // Build the menu when core tells us flags are ready
  document.addEventListener(`${NS}:boot-flags-ready`, async (e) => {
    buildSettingsUI(e.detail);
  });
})();
